<template>
  ...
  <div class="card col-9 m-auto my-5 p-6 ">
    <form @submit.prevent="publishGame">
      ...
      <InputText id="name" v-model="game.name" class="md:w-25rem w-full" placeholder="El título del juego"
                 style="padding: 0.7rem" type="text"/>

      <div class="error-container">
        <small v-for="error of v$.name.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("name", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mb-2 mt-5" for="alias">Alias del juego</label>
      <div class="flex flex-column">
        <InputText id="alias" v-model="game.alias" class="md:w-25rem w-full" placeholder="Alias del juego"
                   style="padding: 0.7rem" type="text"/>
        <small v-if="game.alias" class="mt-1" id="alias-help">Se verá asi en la url: <span
            style="color:#9b2a3a"> {{ toKebabCase(game.alias) }} </span> </small>
      </div>


      <div class="error-container">
        <small v-for="error of v$.alias.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("alias", error.$validator, error) + ". " }}</small>
      </div>
      ...
      <div class="error-container">
        <small v-for="error of v$.description.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("description", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mb-2 mt-5" for="game">Link al github pages</label>
      <InputText id="game" v-model="game.link_to_game" class="w-full" placeholder="El enlace al deploy en github pages"
                 style="padding: 0.7rem" type="text"/>

      <div class="error-container">
        <small v-for="error of v$.link_to_game.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("link_to_game", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mb-2 mt-5" for="game">Link de descarga</label>
      <InputText id="game" v-model="game.link_to_download" class="w-full"
                 placeholder="El enlace de descarga del videojuego" style="padding: 0.7rem" type="text"/>

      <div class="error-container">
        <small v-for="error of v$.link_to_download.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("link_to_download", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mb-2 mt-5" for="devTeam">Equipo desarrollador</label>
      <InputText id="devTeam" v-model="game.development_team" class="w-full "
                 placeholder="El nombre del equipo desarrollador" style="padding: 0.7rem" type="text"/>

      <div class="error-container">
        <small v-for="error of v$.development_team.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("development_team", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mb-2 mt-5" for="release">Fecha de lanzamiento</label>
      <Calendar id="release" v-model="game.release_date" :maxDate="maxReleaseDate" class="w-full input-height"
                dateFormat="dd/mm/yy" placeholder="dd/mm/yyyy" showButtonBar showIcon/>

      <div class="error-container">
        <small v-for="error of v$.release_date.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("release_date", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mb-2 mt-5" for="genres">Géneros</label>
      <MultiSelect id="genres" v-model="game.genres" :maxSelectedLabels="10" :options="genres"
                   class="w-full md:w-25rem input-height align-items-center" display="chip" filter
                   optionLabel="name" placeholder="Seleccionar géneros"/>

      <div class="error-container">
        <small v-for="error of v$.genres.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("genres", error.$validator, error) + ". " }}</small>
      </div>
      ...
      <div class="error-container">
        <small v-for="error of v$.developers.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("developers", error.$validator, error) + ". " }}</small>
      </div>
      ...
      <div class="flex mb-2 gap-3 card p-3 w-fit">
        <div>
          <label class="block text-600 font-medium mb-2" for="devs">Año</label>
          <Calendar v-model="game.period.year" view="year" dateFormat="yy" :min-date="new Date(1960)"
                    :max-date="new Date()" placeholder="Año de cursada"/>
        </div>
        <div>
          <label class="block text-600 font-medium mb-2" for="devs">Cuatrimestre</label>
      ...
      <div class="error-container">
        <small v-for="error of v$.logo_url.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("logo_url", error.$validator, error) + ". " }}</small>
      </div>

      <label class="block text-600 font-medium mt-5 mb-2" for="images">Imágenes</label>
      <GameImageManager id="images" v-model:images="game.images" @remove-image="handleRemoveImage"
                        @add-image="handleAddImage"/>
      <div class="error-container">
        <small v-for="error of v$.images.$errors" :key="error.$uid"
               class="p-error">{{ getCustomError("images", error.$validator, error) + ". " }}</small>
      </div>
      <div class="flex flex-row justify-content-end mt-5 gap-3">
        <Button :loading="false" class="input-height w-8rem cancel-button" label="Cancelar" type="button"
                @click="router.back()"></Button>
        <Button :label="isEditMode ? 'Editar' : 'Publicar'" :loading="false" class="input-height w-8rem"
                type="submit"></Button>
      </div>
    </form>
  </div>
...
const availableSemesters = ref(['Primero', 'Segundo']);

function handleResetEnrollmentData() {
  game.value.period = { }
}
...
async function handleCreation() {
  ...
  toast.add({
    severity: 'success',
    summary: 'Operación exitosa',
    detail: 'Tu juego se ha publicado correctamente',
    life: 3500
  });
  router.push('/dev/games');
}
