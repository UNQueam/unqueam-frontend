...
function handleResize() {
  shouldShowFilters.value = window.innerWidth > 600;
}
...
const resetFilters = () => {
  filterPanel.value.toggle(false)
...
<template>
  ...
  <div class="flex">
    <div
        class="m-auto mt-5 w-100 col-12 md:col-10 lg:col-9 p-4  justify-content-center text-center align-items-center align-content-center">
      <Galleria
          :autoPlay="true"
      ...
      >
        <template #item="slotProps">
          <div class="card p-0 m-0 overflow-auto">
            <Image :alt="slotProps.item.alias" :src="slotProps.item.picture.byte_array_as_string" class="m-auto"
                   style="border-radius: 20px; width: 100%; display: block; cursor: pointer;"
                   @click="goToBanner(slotProps.item)"/>
          </div>
    ...
    <div class="card mb-5 m-auto mt-5 w-100 col-12 md:col-10 lg:col-9 p-4">
      ...
      <DataView :value="filteredData" :layout="layout" :paginator="true" :rows="9">
        <template #header>
          ...
          <div
              v-if="shouldShowFilters && (selectedYears.length > 0 || selectedSemesters.length > 0 || selectedGenres.length > 0)"
              class="font-normal none" style="font-size: 12px">
            ...
            <div v-if="selectedYears.length > 0" class="flex flex-row gap-1 mt-2 align-items-center">
              ...
              <Chip v-for="year of selectedYears" v-bind:key="year" style="font-size: 12px; padding: 2px 5px" removable
                    @remove="removeSelectedYear(year)">{{ year }}
              </Chip>
            </div>
            <div v-if="selectedSemesters.length > 0" class="flex flex-row gap-1 mt-2 align-items-center">
              ...
              <Chip v-for="semester of selectedSemesters" v-bind:key="semester"
                    style="font-size: 12px; padding: 2px 5px" removable @remove="removeSelectedSemester(semester)">
                {{ semester }}
              </Chip>
            </div>
            <div v-if="selectedGenres.length > 0" class="flex flex-row gap-1 mt-2 align-items-center">
              ...
              <Chip v-for="genre of selectedGenres" v-bind:key="genre" style="font-size: 12px; padding: 2px 5px"
                    removable @remove="removeSelectedGenre(genre)">{{ genre }}
              </Chip>
            </div>
          </div>
        </template>
